        ...
        <div class="col-xs-12 col-sm-9" id="handleOperaBox" style="background-color: whitesmoke;margin-top: 10px">
            ...
            <div class="row">
                <c:forEach items="${operaList}" var="opera">
                    <a href="${pageContext.request.contextPath}/AdminManage/handleOperaDetail?operaId=${opera.id}">
                        <div class="col-xs-6 col-lg-4" style="margin-top: 10px;background-color: snow;border-left:1px solid;">
                            <video width="260px" height="200px">
                                <source src="${pageContext.request.contextPath}/${opera.fileName}" type="video/mp4">
                                您的浏览器不支持Video标签。
                            </video>
                            <p><strong>戏曲名：${opera.operaName}</strong></p>
                        </div>
                    </a>
                </c:forEach>
            </div>
...
<script>
    ...
    <%--        //请求后台--%>
    <%--        window.location.href="${pageContext.request.contextPath}/AdminManage/jumpToHandleOperaPage?currentPage"+pageIndex;--%>
    <%--    }--%>
    <%--});--%>
